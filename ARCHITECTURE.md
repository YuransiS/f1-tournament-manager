...
- `src/components/F1StartingGrid.tsx`: Authentic F1 TV broadcast Starting Grid recreation adhering to official TV storyboards (Bahrain 2021 graphics): 4-stage intro sequence (Stage 1: Central `STARTING GRID` metallic pill with red accent ➔ Stage 2: Horizontal visor laser beam expansion with energetic arc pips ➔ Stage 3: Vertical split opening with glowing red `1st` / `2nd` position numbers and rising driver busts ➔ Stage 4: Full television broadcast HUD with metallic chassis bezel, top-left official F1 event badge, driver first & LAST names, country flags, red Chakra Petch positions, standardized bottom-grounded portrait frames, team badges & digital monospace telemetry, plus an authentic central SVG Starting Grid Track Slot diagram showing staggered pole & 2nd brackets with asphalt gridlines). Includes keyboard controls (Space, Arrows, F for fullscreen, R for intro replay).
- `src/components/StartingGridView.tsx`: Interactive view integrating the starting grid widget with real race data filtering from tournament history (`data.races`), Grand Prix selector dropdown, and interval speed controls. Driver avatar mapping relies purely on uniform container fill without per-driver scaling hacks.
