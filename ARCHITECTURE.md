...
- `src/components/F1StartingGrid.tsx`: Authentic F1 TV broadcast Starting Grid recreation adhering to official TV storyboards (Bahrain 2021 broadcast layout):
  - **Dynamic Background:** Ambient host country waving flag GIF (`/flags/animated/${countryCode}.gif`) blended beneath metallic broadcast vignettes with carbon texture.
...
- `src/components/StartingGridView.tsx`: Interactive view integrating the starting grid widget with real race data filtering from tournament history (`data.races`). Automatically resolves Grand Prix host circuit to its ISO country code (`getRaceCountryCode`) to feed the animated waving flag background. Provides Grand Prix selector dropdown, interval speed controls, and authentic qualifying starting grid deltas.
- `public/flags/animated/*.gif`: 17 optimized, local looping waving national flag GIFs for all championship circuits (Bahrain, Japan, Italy, Monaco, Great Britain, USA, etc.).
